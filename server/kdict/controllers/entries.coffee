...
exports.show = (req, res, next) ->
  ...
  Entry.findOne( { 'korean.hangul' : req.params.word } ).populate('tags').exec (err, entry) ->
    return next(new NotFound("Entry not found")) unless entry
    Update.find( { 'entry.id' : entry.id } ).exec (err, updates) ->
      console.log updates
...
exports.showById = (req, res, next) ->
  Entry.findById( req.params.id ).populate('updates').exec (err, entry) ->
    return next(new NotFound("Entry not found")) unless entry
    console.log entry
    res.render 'entries/show',
      # locals:
...
exports.search = (req, res, next) ->
  ...
  async.map pairs, parseParam, (err, results) ->
    ...
    Entry.count(query).limit(paginator.limit).skip(paginator.skip).sort(order).exec (err, count) ->
      if err
        console.log err
        next err
      else
        paginator.setCount count
        Entry.find(query).populate('tags').limit(paginator.limit).skip(paginator.skip).sort(order).exec (err, entries) ->
          if err
            console.log err
            next err
          else
