...
exports.top = (req, res, next) ->
  User.find().sort('score').limit(20).exec (err, users) ->
...
exports.show = (req, res, next) ->
  User.findOne( username: req.params.username ).exec (err, user) ->
    ...
    rawid = user._id
    stringid = user.id
    #Update.find( 'user': user._id ).sort('date').limit(20).exec (err, updates) ->
    ...
    Update.count( user: objid, type: 'new' ).exec (err, added) ->
      Update.count( user: objid, type: 'edit' ).exec (err, edited) ->
        Update.count( user: objid, type: 'delete' ).exec (err, deleted) ->
          Update.count( user: objid, 'status.type': 'approved' ).exec (err, approved) ->
            Update.count( user: objid, 'status.type': 'rejected' ).exec (err, rejected) ->
              Update.count( user: objid, 'status.type': 'pending' ).exec (err, pending) ->
                Update.count( 'status.user': objid, 'status.type': 'approved' ).exec (err, made_approved) ->
                  Update.count( 'status.user': objid, 'status.type': 'rejected' ).exec (err, made_rejected) ->
                    Update.find( user: objid ).sort('created_at').limit(20).exec (err, updates) ->
...
exports.sendResetEmail = (req, res, next) ->
  ...
  User.findOne email: req.body.email, (err, results) ->
    ...
    User.update email: req.body.email,
      ...
      res.render 'sessions/reset_sent', 
      # locals:
...
exports.showResetForm = (req, res, next) ->
  User.find reset_code: token, (err, results) ->
    if err
      console.log err
      res.redirect "/"
    res.render 'sessions/reset_password', 
    # locals:
      title: 'Reset Password'
