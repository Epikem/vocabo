...
exports.index = (req, res, next) ->
  User.find({}).sort('score').limit(10).exec (err, top_users) ->
    return next(err) if err
    Update.find( {type: "new"} ).populate('user').sort('created_at').limit(10).exec (err, new_updates) ->
      return next(err) if err
      Update.find({}).populate('user').sort("created_at").limit(10).exec (err, updates) ->
        console.log updates
        return next(err) if err
        res.render "index",
...
exports.tagged = (req, res) ->
  Tag.find( { type : 'problem' } ).exec (err, problem_tags) ->
    return next(err) if err
